<div class="form-group {{$layout == 'horizontal' ? 'row' : ''}}">
    <label for="text_{{ $name }}" 
           class="{{$layout == 'horizontal' ? 'col-sm-2 col-form-label' : ''}} 
                  {{$radioInline && $layout != 'horizontal'  ? 'd-block' : ''}}
                "
    >
            {{ $label }}
    </label>
        ...
        @endif
            ...
            @foreach ($options as $key => $item)
                <div class="custom-control custom-radio {{ $radioInline ? 'custom-control-inline' : '' }}">
                    <input type="radio" id="text_{{ $name }}_{{$key}}" name="{{$name}}" value="{{ $item['value'] }}" {{ $isChecked(old("$name",$item['value']))}} class="custom-control-input">
